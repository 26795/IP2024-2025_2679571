...
pydic.init('./img/*.png', correl_wind_size, correl_grid_size, "result.dic")


# and read the result file for computing strain and displacement field from the result file 
#pydic.read_dic_file('result.dic', interpolation='spline', strain_type='cauchy', save_image=True, scale_disp=10, scale_grid=25, meta_info_file='img/0303s05sforceOuputInterpsteel2.txt')
...
sample_width     = 0.005
sample_thickness = 0.005
diameter=5/1000
stress = force*1000/(sample_width * sample_thickness)
stress= force*1000/(np.pi*diameter**2/4)
...
plt.plot(ave_strain_xx, stress, label='extension xx')
plt.plot(ave_strain_yy, stress, label='ave_strain_yy')
...
plt.legend() 
print('ave_strain_xx', ave_strain_xx)
print('stress', stress)
...
stressDataframe=pd.DataFrame(stress)
ave_strain_xxDataframe=pd.DataFrame(ave_strain_xx)
ave_strain_yyDataframe=pd.DataFrame(ave_strain_yy)
stressDataframe.to_excel("0304stressDataframe.xlsx")
ave_strain_xxDataframe.to_excel("0304ave_strain_xxDataframe.xlsx")
ave_strain_yyDataframe.to_excel("0304ave_strain_yyDataframe.xlsx")
