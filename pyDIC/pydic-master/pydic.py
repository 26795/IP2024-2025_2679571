...
(ii) smooth the displacement field thanks to the chosen method (raw, linear, spline,etc.)"""
     ...
     def compute_strain_field(self):
          ...
          # here, u is disp_x and v is disp_y
          du_dx, du_dy = np.gradient(self.disp_x, dx, dy, edge_order=2)
          dv_dx, dv_dy = np.gradient(self.disp_y, dx, dy, edge_order=2)

          self.strain_xx = du_dx + .5*( np.power(du_dx, 2) + np.power(dv_dx, 2) )
          self.strain_yy = dv_dy + .5*( np.power(du_dy, 2) + np.power(dv_dy, 2) )
          self.strain_xy = .5*( du_dy + dv_dx + du_dx*du_dy + dv_dx*dv_dy )
...
class Plot:
    ...
    def update(self, val):
        ...
        # self.cb.set_clim(self.smin.val, self.smax.val)
        # self.cb.on_mappable_anged(self.im)
        # self.cb.draw_all() 
