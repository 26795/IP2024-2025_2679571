...
from dialogs import *
from lists import *
from video import VideoLabel
from trackers import TrackingThreadV2
from process import PostProcesserThread
from export import ExportingThread

from display import display_objects, draw_grid
...
class VideoWidget(QWidget):
    def __init__(self):
        ...
        self.LucasKanadePoints=[] # save points for lucas kanade bc it;s a differnet format # change
        ...
        self.pix=[] # display frame
        self.edgeAngle=None #change # in the fucntion change it to 0 if none  so it works evne if not calibrated
        self.timer = (
            QTimer()
    ...
    def initUI(self):
        ...
        with open(os.path.join(os.path.dirname(__file__), "style", "main.qss"), "r") as style:
                  self.setStyleSheet(style.read())
        #self.setWindowIcon(QIcon(os.path.dirname(os.path.dirname(__file__))+"/images/logo.svg"))
        self.setWindowIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','logo.png')))
        ...
        self.StartPauseBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','play.png')))
        self.StartPauseBTN.setStyleSheet("")
        ...
        StopBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','stop.png')))
        ...
        self.ForwardBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','forward.png')))
        ...
        self.BackwardBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','backward.png')))
        ...
        self.StartBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','start.png')))
        ...
        self.EndBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','end.png')))
        ...
        MoveUpBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','up.png')))
        ...
        MoveDownBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','down.png')))
        ...
        MoveLeftBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','left.png')))
        ...
        MoveRightBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','right.png')))
        ...
        ZoomInBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','zoom-in.png')))
        ...
        ZoomOutBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','zoom-out.png')))
        ...
        XLayout = QHBoxLayout()
        XLayout.addWidget(XGridLBL) #X:
        XLayout.addWidget(self.XGridLNE) # line (10) 
        YLayout = QHBoxLayout()
        ...
        self.GridGB.setEnabled(False) # change so it's disabled at the start
        ...
        self.VidLBL.setPixmap(QPixmap(os.path.join(os.path.dirname(__file__), "images", "video.png")))
        ...
        self.VidTimeLBL.setFixedHeight(40) # to stop from distorting the player layout
        ...
        self.setRulerBTN.clicked.connect(self.showEdgeDetectionDialog) #change
        ...
        self.RulerGB.setTitle("Calibrate") # change name
        ...
        self.roiGB.setTitle("Select ROI") #change
        ...
        self.TrackBTN_py = QPushButton()
        self.TrackBTN_py.setText("Export images")
        self.TrackBTN_py.setEnabled(True)
        self.TrackBTN_py.setObjectName("trackBTN")
        self.TrackBTN_py.clicked.connect(self.exportImage)
        
        trackLayout=QVBoxLayout()
        trackLayout.addWidget(self.TrackBTN_py)
        trackLayout.addWidget(self.TrackBTN)
        ...
        #inisert here from hte right side
        LSideLayout.addWidget(self.ObjectsGB)
        
        LSideLayout.addItem(QSpacerItem(0, 10, QSizePolicy.Maximum))

        LSideLayout.addWidget(self.GridGB)
        LSideLayout.addWidget(self.ZoomControlGB)
        
        
        LSideLayout.addItem( QSpacerItem(0, 10, QSizePolicy.Maximum, QSizePolicy.Expanding))
        LSideLayout.addWidget(self.reProcessBTN)
        #LSideLayout.addWidget(TrackBTN)
        LSideLayout.addLayout(trackLayout)
        #LSideLayout.setContentsMargins(0, 0, 0, 12)
        ...
        #insert from Rside
        # LSideLayout.addWidget(self.exportBTN)
        # LSideLayout.addWidget(self.playbackGB)
        # LSideLayout.addWidget(self.exportVidBTN)
        # LSideLayout.addItem( QSpacerItem(0, 10, QSizePolicy.Maximum, QSizePolicy.Expanding))
        ...
        #VideoControlLayout.addWidget(self.VidTimeLBL)
        #PlayerControlLayout.addWidget(self.VidTimeLBL) #timer
       ...
       # self.edgeDetectionDialog=edgeDetection(self.frameEdges) #change
    ...
    def closeVideo(self):
        ...
        self.PropGB.setVisible(False)
        self.VidTimeLBL.setText("-/-")
        self.VidLBL.setPixmap(QPixmap(os.path.dirname(__file__)+"/images/video.png"))
    ...
    def StartPauseVideo(self):
        ...
        if self.StartPauseBTN.isChecked():
            ...
            self.StartPauseBTN.setIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','pause.png')))
        else:
    ...
    def nextFrame(self):
        ...
        if ret:
            self.frameEdges=frame # change # otehr changes to frame downstream, saves this cv2 frame before cghagngin 

            # Grid
            if self.GridGB.isChecked() and self.edgeAngle is not None: # change # needs both to be cheked and have an angle for grid drawing
                frame = draw_grid(
                    int(self.XGridLNE.text()), # number of x lines
                    int(self.YGridLNE.text()), # unumebr of y lines 
                    frame,
                    self.GridColorCMB.currentText(), #selected grid colour 
                    self.edgeAngle, # add edge angle  # change
                )
    ...
    def ReloadCurrentFrame(self):
        ...
        if ret:
            self.frameEdges=frame # change 
            # Grid
            if self.GridGB.isChecked() and self.edgeAngle is not None:  #change so both conditiosn are met
                frame = draw_grid(
                    int(self.XGridLNE.text()),
                    int(self.YGridLNE.text()),
                    frame,
                    self.GridColorCMB.currentText(),
                    self.edgeAngle, # change
                )
    ...
    def saveObject(self):
        ...
        #change #save into lucas kanade point array
        self.LucasKanadePoints.append(self.point_tmp)
        print('lucas kanade array', self.LucasKanadePoints)
        # delete temp buffer
        self.point_tmp = None
        self.rect_tmp = None
    ...
    def savePoint(self, x, y):
        ...
        # change # check in there are any poitns saved previosly
        if self.objects_to_track != []:
            # now we need to extravt the preiosu poitn coordiantes
            #previosu poitn cooridnates
            
            
            print('not the first point') 
            print( 'all objects', self.objects_to_track)  
            print( 'full object', self.objects_to_track[-1])
            print( 'coridnates?',  self.objects_to_track[-1].point)  # extract the last object added # find point value from the class it was saved with  
            
            #change
            # coordanites of a previous point # first indiacte object # when idicate variable 
            # first point where it was placed, second point moved to be on the line paraller to the edge
            # either x or y remaisn as the user eelcted, the other coordinate is cacl using the edge angle
            x_prev, y_prev = self.objects_to_track[-1].point
            xChange= abs(x-x_prev)
            yChange= abs(y-y_prev)
            print('x,y', x,y)
            print('dx', xChange, 'dy', yChange)
            if self.edgeAngle is None:
                self.edgeAngle=0
            if xChange>=yChange:
                print('horizontal motion')
                x=x # no change in x vlaue, moves as a line
                y= int(y_prev+ (x-x_prev)*np.tan(np.deg2rad(self.edgeAngle))) #y+dy=y+dx*tana
                print('x,y', x,y) 
            else:
                print('vertical motion')
                x= int(x_prev+(y-y_prev)*np.tan(np.deg2rad(self.edgeAngle)))
                y=y
                print('x,y', x,y) 
        else: 
            print('first point') 
        ...
        self.point_tmp = (x, y) 
        print('coordiantes temprorary ', self.point_tmp)
    ...
    def setRuler(self):
        ...
        #draws the ruler ++ using the angle derived
        # connect signals
    ...
    #export images for pyDIC #change
    def exportImage(self):
         print('0 start image export')
         startframe=self.section_start
         fps=self.fps
         endframe=self.section_stop
         print('1',startframe, endframe, fps)
         deltaT=0.5
         deltaTinaSec=1/deltaT
         frameinterval=int(fps/deltaTinaSec)
         print('2',deltaT,deltaTinaSec,frameinterval)
         frameDifference=endframe-startframe
         remainder=frameDifference%frameinterval
         print('3',frameDifference, remainder)
         if remainder !=0:
             print('3.2',startframe)
             startframe=startframe+remainder
             print('3.4',startframe)
         ...
         while currentframe<=endframe:
             self.camera.set(cv2.CAP_PROP_POS_FRAMES, currentframe) 
             # Read the frame from the video
             ret, frame = self.camera.read()
             if not ret:
                 print("Error reading frame")
                 break
             #cv2.imwrite(f'03 03 images for piDIC/steel{n:04d}.png', frame)
             cv2.imwrite(f'03 20 steel frame tests/steel{n:04d}.png', frame)
             #namearray.append(f'steel{n:04d}.png')
             namearray.append(f'steel{n:04d}.png') #make sure that infort are zeroes bc the app doesnt like it ohterwise
             
             n=n+1
             currentframe=frameinterval+currentframe
             print(n)
         print('4',currentframe, endframe, namearray, n)
         namearray=pd.DataFrame(namearray, columns=['file'])
         #namearray.to_excel('03 03 images for piDIC/steelFileNames.xlsx')
         namearray.to_excel('03 20 steel frame tests/steelFileNames.xlsx')
         print('finish')
    ...
    def runTracker(self, tracker_type, size, fps):
        ...
        self.tracker = TrackingThreadV2(
            ...
            self.timestamp,
            self.roi_rect,
            self.LucasKanadePoints #change incluce lucas kanade
        )
    ...
    def showErrorMessage(self, message):
        ...
        with open(os.path.join(os.path.dirname(__file__), "style", "message.qss"), "r") as style: 
            msg.setStyleSheet(style.read())
        #msg.setWindowIcon(QIcon(os.path.dirname(os.path.dirname(__file__))+"/images/logo.svg"))
        msg.setWindowIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','logo.png')))
...
#        self.rotGB.setEnabled(True)
    ...
    def showEdgeDetectionDialog(self): #change
        #shpw edge detection dialog
        if self.frameEdges is not None:
            # initializing dialogs
            self.edgeDetectionDialog=edgeDetection(self.frameEdges, self.VidLBL) #change
            self.edgeDetectionDialog.show()
            result = self.edgeDetectionDialog.exec_()
            
            
            if result == QDialog.Accepted:
                selected_angle = self.edgeDetectionDialog.result_angle
                if selected_angle is not None:
                    self.edgeAngle = selected_angle

                self.GridGB.setEnabled(True)
            
            self.ReloadCurrentFrame()# update the frame
