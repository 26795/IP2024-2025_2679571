...
class PostProcessSettings(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        with open(os.path.join(os.path.dirname(__file__), 'style','postprocess.qss'), "r") as style:
...
class TrackingSettings(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        with open(os.path.join(os.path.dirname(__file__), 'style','tracking.qss'), "r") as style:
            self.setStyleSheet(style.read())
        self.setWindowIcon(QIcon(os.path.dirname(os.path.dirname(__file__))+"/images/logo.svg"))
        ...
        self.algoCMB.addItems(
            ['Pyramidal Lucas Kanade', "CSRT"] #change
        )
       # self.algoCMB.addItems(
       #     ['Pyramidal Lucas Kanade', "CSRT", "BOOSTING", "MIL", "KCF", "TLD", "MEDIANFLOW", "MOSSE"] #change
       # )
...
class TrackingProgress(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowIcon(QIcon(os.path.dirname(os.path.dirname(__file__))+"/images/logo.svg"))
        with open(os.path.join(os.path.dirname(__file__), 'style','progress.qss'), "r") as style:
            self.setStyleSheet(style.read())
...
class ExportDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # styling
        self.setWindowFlags(self.windowFlags() ^ Qt.WindowContextHelpButtonHint)
        self.setWindowIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','logo.svg'))) #change
        self.setObjectName("export_dialog")
        #with open(os.path.dirname(os.path.dirname(__file__))+"/style/export.qss", "r") as style:
        with open(os.path.join(os.path.dirname(__file__), 'style','export.qss'), "r") as style:
            self.setStyleSheet(style.read())
...
class edgeDetection(QDialog): #change 
    #modal dialog for finding the angle the edge makes with the horizontal
    
    def __init__(self, frameEdges, VidLBL, parent=None):
        """Initialization"""
        ...
        with open(os.path.join(os.path.dirname(__file__), "style", "main.qss"), "r") as style:
                  self.setStyleSheet(style.read())
        #self.setWindowIcon(QIcon(os.path.dirname(os.path.dirname(__file__))+"/images/logo.svg"))
        self.setWindowIcon(QIcon(os.path.join(os.path.dirname(__file__),'images','logo.svg')))
        ...
        # sliders to adjust canny edge detection threshold values
        self.Threshold1Slider = QSlider(Qt.Horizontal)
        self.Threshold1Slider.setGeometry(50, 320, 500, 20)
        self.Threshold1Slider.setRange(0, 600)  # Set range for the first slider
        self.Threshold1Slider.setValue(200)  
        self.Threshold1Slider.valueChanged.connect(self.DetectEdges_Canny) # connects ot the fucntion whenether a change happens

        
        self.Threshold2Slider = QSlider(Qt.Horizontal)
        self.Threshold2Slider.setGeometry(50, 350, 500, 20)
        self.Threshold2Slider.setRange(0, 600)  # Set range for the second  slider
        self.Threshold2Slider.setValue(400)  # preliminary value 
        self.Threshold2Slider.valueChanged.connect(self.DetectEdges_Canny)

        # Layout for sliders
        slidersLayout = QVBoxLayout()
        slidersLayout.addWidget(self.Threshold1Slider)
        slidersLayout.addWidget(self.Threshold2Slider)
        
        #buttons       
        
        self.okButton = QPushButton('Select edge') # slect edge then changes into 
        self.okButton.setGeometry(150, 380, 100, 40)
        self.okButton.setStyleSheet("font-size: 14px; padding: 5px;")
        self.okButton.clicked.connect(self.okButtonClick)

        self.cancelButton = QPushButton('Cancel')
        self.cancelButton.setGeometry(350, 380, 100, 40)
        self.cancelButton.setStyleSheet("font-size: 14px; padding: 5px;") # so buttons look good 
        ...
        #overall winbdow layout 
        windowLayout=QVBoxLayout()
        windowLayout.addWidget(self.videoLabel)
        ...
        #save the frame
        self.frame=frameEdges
        #save video size
        self.VidLBL=VidLBL
        
        
        #run canny edges once so it has a frame to display 
        self.DetectEdges_Canny()
    ...
    def DetectEdges_Canny(self):
       
       #frame sizing factor
       a=0.95
       
       minThreshold = self.Threshold1Slider.value()
       maxThreshold = self.Threshold2Slider.value()
       
       #detect edges
       edges = cv.Canny(self.frame,minThreshold,maxThreshold)
       self.edges=edges # frame with white edge lines
       
       # convert from cv2 into pixmap and display the current frame
       #  color
       ...
       pix = pix.scaled(
             int(self.VidLBL.width()*a), #change
             int(self.VidLBL.height()*a), #chagne
             Qt.KeepAspectRatio,
             Qt.SmoothTransformation,)
       
       
       self.videoLabel.setPixmap(pix) # move into the label     
       
        
    def eventFilter(self, source, event): # mouse event to select an edge 
        
        if source == self.videoLabel and event.type() == event.MouseButtonPress:
            x, y = event.pos().x(), event.pos().y()
            
            searchWindow = 100 # square for white pixel search 
            edge_found = False
            
            for dx in range(-searchWindow, searchWindow + 1):  # move through search window 
                for dy in range(-searchWindow, searchWindow + 1):
                    nx, ny = x + dx, y + dy  # search point coordiantes 
                    if self.edges[ny, nx] == 255:  # if pixel is white
                        edge_found = True
                        break   
                if edge_found:
                    break
            ...
            if edge_found:
                ...
                print("point not on edge")
        return super().eventFilter(source, event)

    def draw_line(self): # draw line,  calcualte angle
        if len(self.selected_points) == 2:
            (x1, y1), (x2, y2) = self.selected_points
            painter = QPainter(self.videoLabel.pixmap())
            ...
            print(f"Edge Angle: {angle:.2f} deg")
            self.selected_angle = angle 
        
    def okButtonClick(self):
        if self.okButton.text() == 'Select edge':
            #disable sliders
            self.Threshold1Slider.setEnabled(False)
            self.Threshold2Slider.setEnabled(False)
            #chagne button text
            self.okButton.setText('OK')
        else:
            if self.selected_angle is not None: # if there is an angle to return 
                self.result_angle = self.selected_angle 
            self.accept()
