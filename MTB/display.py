...
def draw_grid(x_num, y_num, frame, color_name, edgeAngle): # change add a variable 
    """Draws grid onto the frame with the given parameters"""
    dx = frame.shape[1] / x_num # pixel difference between two lines 
    dy = frame.shape[0] / y_num
    edgeAngle=np.deg2rad(edgeAngle) # into rad
    ...
    for x in np.linspace(start=dx, stop=frame.shape[1] - dx, num=x_num): # veritcal lines
        x = int(round(x))
        # cv2.line(
        #     frame, (x, 0), (x, frame.shape[0]), color=color, thickness=1,  # (x, frame.shape[0]) resutls in a sragnth line, from y=0 to y= max
        # )
        #Change
        #intial point
        start_x=x # same as before # intial point
        start_y=0 # initial poitn coordiantes
        #end point coordinates 
        end_x = int(x + np.tan(edgeAngle)*frame.shape[0])
        end_y = frame.shape[0] 
        #drawline
        cv2.line(
            frame, (start_x, start_y), (end_x, end_y), color=color, thickness=1,  #make it at an 
        ) 
    for y in np.linspace(start=dy, stop=frame.shape[0] - dy, num=y_num): #horizontal lines
        y = int(round(y))
        # cv2.line(
        ...
        #end point coordinates 
        end_x = frame.shape[1]
        end_y = int(y+ np.tan(edgeAngle)*frame.shape[1])
        #drawline
        cv2.line(
            frame,  (start_x, start_y), (end_x, end_y), color=color, thickness=1,
        )
    return frame